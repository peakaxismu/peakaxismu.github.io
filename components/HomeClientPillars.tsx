      ...
      <div className="wrap">
        <div className="tab-row" role="tablist" aria-label="Experience Pillars">
          {[
            ...
            <button key={tab} id={`tab-${tab}`} role="tab" aria-selected={activeTab === tab} aria-controls={`panel-${tab}`} className="" onClick={() => setActiveTab(tab as typeof activeTab)}>
        ...
        <div id="panel-expeditions" role="tabpanel" aria-labelledby="tab-expeditions" className="">
          <div className="p-left">
            ...
            <div className="p-ctas"><Link href="/expeditions" className="btn-primary">Explore Piton de la Fournaise</Link></div>
          </div>
        ...
        <div id="panel-team" role="tabpanel" aria-labelledby="tab-team" className="">
          <div className="p-left"><h2>Teams work better after they&apos;ve climbed something together.</h2><p>Custom outdoor challenges designed for company teams. From ridge walks to orientering tasks that build trust, clear minds, and test problem-solving outside the office.</p><div className="p-ctas"><Link href="/enquire?interest=team" className="btn-primary">Enquire for your team</Link></div></div>
          <div className="p-right"><div className="card-list">{outdoorTeam.slice(0, 3).map((pkg) => <div key={pkg.id} className="mini-card"><div className="tag">OUTDOOR</div><h3>{pkg.name}</h3><div className="meta">{pkg.description}</div></div>)}</div></div>
        </div>

        <div id="panel-activities" role="tabpanel" aria-labelledby="tab-activities" className="">
          <div className="p-left"><h2>Indoor and outdoor, for any group.</h2><p>Tailored group experiences — half-day workshops, nature walks, and adventure sessions for schools, clubs, or private gatherings.</p><div className="p-ctas"><Link href="/enquire?interest=activity" className="btn-primary">Plan a custom activity</Link></div></div>
          <div className="p-right"><div className="card-list">{indoorActivities.slice(0, 3).map((act) => <div key={act.id} className="mini-card"><div className="tag">INDOOR / OUTDOOR</div><h3>{act.name}</h3><div className="meta">{act.description}</div></div>)}</div></div>
        </div>
      </div>
    </section>
  )
