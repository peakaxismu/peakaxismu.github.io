...
export default function HikesClientList({ hikes }: { hikes: Hike[] }) {
  ...
  const getNumericDifficulty = (value: string) => {
  ...
  const getDiffBadge = (diff: string, numeric: string) => {
    ...
    return <span className={`diff ${className}`}>{label} · {numeric}</span>
  }

  const filteredHikes = useMemo(() => {
    const result = hikes.filter((hike) => {
      if (difficultyFilter !== 'all' && hike.difficulty.toLowerCase() !== difficultyFilter) return false
      if (typeFilter !== 'all' && !hike.hike_type.toLowerCase().includes(typeFilter)) return false

      const numeric = getNumericDifficulty(hike.difficulty_numeric)
      if (intensityFilter === 'easy' && (numeric === null || numeric > 4)) return false
      if (intensityFilter === 'moderate' && (numeric === null || numeric < 5 || numeric > 6)) return false
      if (intensityFilter === 'challenging' && (numeric === null || numeric < 7)) return false
      return true
    })

    return [...result].sort((a, b) => {
      if (sortBy === 'overall') return b.overall_rating - a.overall_rating
      if (sortBy === 'scenery') return b.scenery_rating - a.scenery_rating
      return 0
    })
  ...
  const getSvgVisual = (index: number) => {
    ...
    return (
      <svg viewBox="0 0 200 140" aria-hidden="true">
        <path d={paths[index % paths.length]} fill="#1F4B4C" />
      </svg>
    )
  }
  ...
  return (
    <div className="wrap">
      <section className="hike-intro-grid" aria-label="How Peak Axis hikes work">
        <div><span>01</span><strong>Choose your terrain</strong><p>Filter by difficulty, route type or intensity and find a hike that fits the day you want.</p></div>
        <div><span>02</span><strong>See the essentials</strong><p>Every listing shows the route type, duration, location, difficulty, ratings and current availability.</p></div>
        <div><span>03</span><strong>Ask or book</strong><p>Choose a scheduled hike or switch to a private route if you want your own date and group.</p></div>
      </section>

      <div className="filters">
        {['all', 'easy', 'moderate', 'challenging'].map((value) => (
          <button key={value} className={`fbtn ${difficultyFilter === value ? 'active' : ''}`} onClick={() => setDifficultyFilter(value)}>
            {value[0].toUpperCase() + value.slice(1)}
          </button>
        ))}
        <select aria-label="Filter by hike type" value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)}>
          {typeOptions.map((type) => <option key={type} value={type}>{type === 'all' ? 'All types' : type[0].toUpperCase() + type.slice(1)}</option>)}
        </select>
        <select aria-label="Filter by intensity" value={intensityFilter} onChange={(e) => setIntensityFilter(e.target.value)}>
          <option value="all">All intensity</option>
          <option value="easy">1–4 / 10</option>
          <option value="moderate">5–6 / 10</option>
          <option value="challenging">7–10 / 10</option>
        </select>
        <select aria-label="Sort hikes" value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
          <option value="created">Sort: Default</option>
          <option value="overall">Overall rating ↓</option>
          <option value="scenery">Scenery rating ↓</option>
        </select>
      </div>

      <div className="private-banner">
        <div>
          <span className="banner-kicker">PRIVATE ADVENTURE</span>
          <h2>Want your own trail, your own date?</h2>
          <p>Book a private hike for your group — choose the trail and preferred date instead of joining a scheduled one.</p>
        </div>
        <Link href="/enquire?interest=private_hike" className="btn-ghost">Book a private hike</Link>
      </div>
      ...
      <div className="hike-list" id="hikeList">
        {filteredHikes.map((hike, idx) => {
          ...
          const available = (hike.spots_remaining ?? 0) > 0

          return (
            <article key={hike.id} className="hike-row" data-diff={hike.difficulty.toLowerCase()}>
              <Link href={detailHref} className="hike-visual" aria-label={`View ${hike.name}`}>{getSvgVisual(idx)}</Link>
              <div className="hike-info">
                <div className="hike-info-top">
                  <span className="hike-kicker">MAURITIUS · GUIDED HIKE</span>
                  {isLowSpots && <span className="availability-hot">Limited places</span>}
                </div>
                <h3><Link href={detailHref}>{hike.name}</Link></h3>
                <div className="hike-meta">
                  {getDiffBadge(hike.difficulty, hike.difficulty_numeric)}
                  <span>{hike.hike_type}</span>
                  <span>{hike.duration}</span>
                  <span>{hike.location}</span>
                </div>
                <p className="hike-attraction">{hike.main_attraction}</p>
                {hike.description && <p className="hike-description">{hike.description}</p>}
                <div className="hike-ratings"><span>Scenery <strong>{hike.scenery_rating}/10</strong></span><span>Overall <strong>{hike.overall_rating}/10</strong></span></div>
                <Link href={detailHref} className="detail-link">View hike details →</Link>
              </div>
              <div className="hike-action">
                <span className="action-label">GROUP RATE</span>
                <div className="hike-price">${hike.price_group_usd}<span>/person</span></div>
                <div className="solo-rate">Solo ${hike.price_solo_usd}</div>
                <div className={`spots ${isLowSpots ? 'low' : ''}`}>{available ? `${hike.spots_remaining} spots left` : 'Fully booked'}</div>
                <Link href={available ? `/enquire?interest=hike&ref=${encodeURIComponent(hike.name)}` : '/enquire?interest=private_hike'} className="btn-primary">{available ? 'Book your spot' : 'Ask about a private hike'}</Link>
              </div>
            </article>
          )
        })}
