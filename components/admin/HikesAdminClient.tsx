...
const inputStyle = { width:'100%', padding:'11px 12px', border:'1px solid var(--sand-line)', background:'var(--warm-white)', color:'var(--ink)', borderRadius:2, fontSize:14, outline:'none' }
...
export default function HikesAdminClient({ initialHikes }: { initialHikes: Hike[] }) {
  ...
  const visibleHikes=useMemo(()=>hikes.filter(h=>{
    ...
    const matchesSearch=!q||[h.name,h.location,h.region,h.hike_type,h.main_attraction].filter(Boolean).some(v=>v!.toLowerCase().includes(q))
  ...
  const stats=useMemo(()=>({total:hikes.length,published:hikes.filter(h=>h.status==='published').length,drafts:hikes.filter(h=>h.status==='draft').length,onDemand:hikes.filter(h=>h.booking_type!=='scheduled_group').length}),[hikes])
  ...
  const closeModal=()=>{
    ...
    setIsModalOpen(false);setEditingHike(null);setOriginalHike(null);setError('')
  }
  ...
  const openCreateModal=()=>{
    const value:Partial<Hike>={name:'',difficulty:'moderate',date:'',duration:'',location:'',price:'',spots_total:10,spots_remaining:10,description:'',status:'draft',booking_type:'on_demand',what_to_bring:[],included:[],excluded:[],experience_types:[],rating_label:'Peak Axis rating'}
    setEditingHike(value);setOriginalHike(value);setActiveTab('basics');setError('');setIsModalOpen(true)
  }
  const openEditModal=(h:Hike)=>{const value={...h};setEditingHike(value);setOriginalHike(value);setActiveTab('basics');setError('');setIsModalOpen(true)}
  const set=(key:string,value:unknown)=>setEditingHike(p=>p?{...p,[key]:value}:p)

  const handleSave=async(e:React.FormEvent)=>{
    e.preventDefault();if(!editingHike)return
    setLoading(true);setError('')
    try{
      const isEdit=Boolean(editingHike.id)
      const res=await fetch(isEdit?`/api/admin/hikes/${editingHike.id}`:'/api/admin/hikes',{method:isEdit?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(editingHike)})
      const json=await res.json().catch(()=>({}))
      if(!res.ok||!json.success||!json.data?.[0]){setError(json.error||'Could not save this hike. Please check the fields and try again.');return}
      const saved=json.data[0] as Hike
      setHikes(p=>isEdit?p.map(h=>h.id===saved.id?saved:h):[saved,...p]);setIsModalOpen(false);setEditingHike(null);setOriginalHike(null)
    }catch(err){console.error(err);setError('Network error. Please try again.')}finally{setLoading(false)}
  }

  const handleDelete=async(id:string)=>{
    if(!window.confirm('Delete this hike permanently? This cannot be undone.'))return
    setDeletingId(id)
    try{const res=await fetch(`/api/admin/hikes/${id}`,{method:'DELETE'});if(res.ok)setHikes(p=>p.filter(h=>h.id!==id));else window.alert('Could not delete the hike.')}catch{window.alert('Network error while deleting.')}finally{setDeletingId(null)}
  }
  ...
  const toggleStatus=async(h:Hike)=>{
    ...
    try{
      ...
      const json=await res.json().catch(()=>({}))
      if(res.ok&&json.success){const saved=(json.data?.[0]||{...h,status}) as Hike;setHikes(p=>p.map(x=>x.id===h.id?saved:x))}else window.alert(json.error||'Could not update status.')
  ...
  return <div className="admin-hikes">
    ...
    <div className="table-card"><div className="table-wrap"><table className="admin-table"><thead><tr><th>Route</th><th>Difficulty</th><th>Format</th><th>Price / capacity</th><th>Status</th><th>Actions</th></tr></thead><tbody>{visibleHikes.map(h=><tr key={h.id}><td><div className="route-name">{h.name}</div><div className="route-meta">📍 {h.location}{h.region?` · ${h.region}`:''}</div></td><td><span className={`diff diff-${h.difficulty}`}>{h.difficulty}{h.difficulty_numeric?` · ${h.difficulty_numeric}`:''}</span></td><td><div className="format">{formatBooking(h)}</div><div className="muted">⏱ {h.duration||'Duration not set'}</div></td><td><div className="format">{h.price||'Price not set'}</div><div className="muted">{h.spots_remaining ?? 0} / {h.spots_total ?? 0} spots</div></td><td><button className={`status-btn status-${h.status}`} title={`Switch to ${h.status==='published'?'draft':'published'}`} onClick={()=>toggleStatus(h)}>{h.status}</button></td><td><div className="actions"><button className="action-btn" onClick={()=>openEditModal(h)}>Edit</button><button className="action-btn delete-btn" disabled={deletingId===h.id} onClick={()=>handleDelete(h.id)}>{deletingId===h.id?'…':'Delete'}</button></div></td></tr>)}</tbody></table></div>{visibleHikes.length===0&&<div className="empty"><strong>No hikes found</strong>Try a different search or clear your filters.</div>}</div>

    {isModalOpen&&editingHike&&<div className="modal-backdrop" role="dialog" aria-modal="true" aria-label={editingHike.id?'Edit hike':'Create hike'} onMouseDown={e=>{if(e.target===e.currentTarget)closeModal()}}><div className="modal">
      <div className="modal-head"><div><div className="modal-title">{editingHike.id?'Edit hike':'Create hike'}</div><div className="modal-help">Complete verified customer-facing details. Unknown trail facts can stay blank.</div></div><button className="close" aria-label="Close" onClick={closeModal}>×</button></div>
      <nav className="tabs" aria-label="Hike editor sections">{tabs.map(t=><button key={t.id} type="button" className={`tab ${activeTab===t.id?'active':''}`} onClick={()=>setActiveTab(t.id)}><strong>{t.label}</strong><span>{t.hint}</span></button>)}</nav>
