...
export async function PUT(
  ...
  try {
    ...
    if ((name as string).length > 200 || (type as string).length > 100 || (description as string).length > 10000) return NextResponse.json({ error: 'One or more fields are too long' }, { status: 400 })
