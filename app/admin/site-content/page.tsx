...
type Testimonial = { id: string; quote: string; name: string; activity: string; status: 'draft' | 'published'; sort_order: number; photo_url?: string | null; photo_position_x?: number; photo_position_y?: number; photo_zoom?: number }
...
export default function SiteContentAdminPage() {
  ...
  useEffect(() => { if (!photoFile) { setPhotoPreview(null); return }; const url = URL.createObjectURL(photoFile); setPhotoPreview(url); return () => URL.revokeObjectURL(url) }, [photoFile])

  const startPhotoDrag = (e: React.PointerEvent<HTMLDivElement>) => {
  ...
  const movePhotoDrag = (e: React.PointerEvent<HTMLDivElement>) => {
    if (!photoDragRef.current || !editingTestimonial) return
    const dx = e.clientX - photoDragRef.current.x
    const dy = e.clientY - photoDragRef.current.y
    photoDragRef.current = { x: e.clientX, y: e.clientY }
    const x = Math.max(0, Math.min(100, (editingTestimonial.photo_position_x ?? 50) - dx * 0.8))
    const y = Math.max(0, Math.min(100, (editingTestimonial.photo_position_y ?? 30) - dy * 0.8))
    setEditingTestimonial({ ...editingTestimonial, photo_position_x: Math.round(x), photo_position_y: Math.round(y) })
  }
  const endPhotoDrag = () => { photoDragRef.current = null }
  const zoomPhoto = (e: React.WheelEvent<HTMLDivElement>) => {
    if (!editingTestimonial) return
    e.preventDefault()
    const next = Math.max(1, Math.min(3, (editingTestimonial.photo_zoom ?? 1) + (e.deltaY < 0 ? 0.1 : -0.1)))
    setEditingTestimonial({ ...editingTestimonial, photo_zoom: Number(next.toFixed(2)) })
  }
  ...
  const saveTestimonial = async (e: React.FormEvent) => {
    ...
    try {
      let payload = { ...editingTestimonial }
      if (photoFile) {
        const form = new FormData()
        form.append('file', photoFile)
        const upload = await fetch('/api/admin/testimonials/upload', { method: 'POST', body: form })
        const uploaded = await upload.json()
        if (!upload.ok) throw Error(uploaded.error || 'Failed to upload photo')
        payload = { ...payload, photo_url: uploaded.url }
      }
      const id = payload.id
      const r = await fetch(id ? `/api/admin/testimonials/${id}` : '/api/admin/testimonials', { method: id ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      const j = await r.json(); if (!r.ok) throw Error(j.error || 'Failed to save testimonial')
      setPhotoFile(null); setPhotoPreview(null); setEditingTestimonial(null); await load(); setMessage('Testimonial saved.')
  ...
  return <div className="page">
    ...
    {editingTestimonial && <div className="modal"><form onSubmit={saveTestimonial} className="modal-card"><div className="modal-head"><div><div className="eyebrow">TESTIMONIAL</div><h2>{editingTestimonial.id ? 'Edit testimonial' : 'Add testimonial'}</h2></div><button type="button" className="icon" onClick={() => { setPhotoFile(null); setPhotoPreview(null); setEditingTestimonial(null) }}>×</button></div><label>Quote<textarea required rows={6} value={editingTestimonial.quote || ''} onChange={e => setEditingTestimonial({ ...editingTestimonial, quote: e.target.value })} /></label><label>Name<input required value={editingTestimonial.name || ''} onChange={e => setEditingTestimonial({ ...editingTestimonial, name: e.target.value })} /></label><label>Activity<input required value={editingTestimonial.activity || ''} onChange={e => setEditingTestimonial({ ...editingTestimonial, activity: e.target.value })} /></label><div className="photo-editor"><div className="photo-preview-wrap"><div className="photo-preview" onPointerDown={startPhotoDrag} onPointerMove={movePhotoDrag} onPointerUp={endPhotoDrag} onPointerCancel={endPhotoDrag} onWheel={zoomPhoto} title="Drag to position · scroll to zoom">{photoPreview || editingTestimonial.photo_url ? <img src={photoPreview || editingTestimonial.photo_url || ''} alt="" style={{objectPosition:(editingTestimonial.photo_position_x ?? 50) + '% ' + (editingTestimonial.photo_position_y ?? 30) + '%',transform:'scale(' + (editingTestimonial.photo_zoom ?? 1) + ')'}} /> : <span>{editingTestimonial.name ? editingTestimonial.name.split(/\s+/).map(x => x[0]).join('').slice(0,2).toUpperCase() : 'PHOTO'}</span>}</div><small className="photo-hint">Drag to move · scroll to zoom</small></div><div className="photo-controls"><label>Photo<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={e => setPhotoFile(e.target.files?.[0] || null)} /><small>JPG, PNG, WEBP or GIF · max 5MB</small></label><label>Horizontal position <span className="optional">{editingTestimonial.photo_position_x ?? 50}%</span><input type="range" min="0" max="100" value={editingTestimonial.photo_position_x ?? 50} onChange={e => setEditingTestimonial({ ...editingTestimonial, photo_position_x: Number(e.target.value) })} /></label><label>Vertical position <span className="optional">{editingTestimonial.photo_position_y ?? 30}%</span><input type="range" min="0" max="100" value={editingTestimonial.photo_position_y ?? 30} onChange={e => setEditingTestimonial({ ...editingTestimonial, photo_position_y: Number(e.target.value) })} /></label><label>Zoom <span className="optional">{editingTestimonial.photo_zoom ?? 1}×</span><input type="range" min="1" max="3" step="0.05" value={editingTestimonial.photo_zoom ?? 1} onChange={e => setEditingTestimonial({ ...editingTestimonial, photo_zoom: Number(e.target.value) })} /></label></div></div><div className="field-grid compact"><label>Status<select value={editingTestimonial.status || 'draft'} onChange={e => setEditingTestimonial({ ...editingTestimonial, status: e.target.value as 'draft' | 'published' })}><option value="draft">Draft</option><option value="published">Published</option></select></label><label>Display order<input type="number" min="0" value={editingTestimonial.sort_order ?? 0} onChange={e => setEditingTestimonial({ ...editingTestimonial, sort_order: Number(e.target.value) })} /></label></div><div className="modal-actions"><button type="button" className="secondary" onClick={() => setEditingTestimonial(null)}>Cancel</button><button className="primary" disabled={saving}>{saving ? 'Saving…' : 'Save testimonial'}</button></div></form></div>}
