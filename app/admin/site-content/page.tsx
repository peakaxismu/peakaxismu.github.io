...
export default function SiteContentAdminPage() {
  ...
  const endPhotoDrag = (e?: React.PointerEvent<HTMLDivElement>) => {
  ...
  const zoomPhoto = (e: React.WheelEvent<HTMLDivElement>) => {
    ...
    setEditingTestimonial(prev => {
      if (!prev) return prev
      const next = Math.max(1, Math.min(3, (prev.photo_zoom ?? 1) + (e.deltaY < 0 ? 0.1 : -0.1)))
      return { ...prev, photo_zoom: Number(next.toFixed(2)) }
    })
  }
