...
type Testimonial = { id: string; quote: string; name: string; activity: string; status: 'draft' | 'published'; sort_order: number; photo_url?: string | null; photo_position_x?: number; photo_position_y?: number }
...
export default function SiteContentAdminPage() {
  ...
  const saveTestimonial = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPhotoFile(null); setEditingTestimonial(null); await load(); setMessage('Testimonial saved.')
  ...
  return <div className="page">
    ...
    {editingTestimonial && <div className="modal"><form onSubmit={saveTestimonial} className="modal-card"><div className="modal-head"><div><div className="eyebrow">TESTIMONIAL</div><h2>{editingTestimonial.id ? 'Edit testimonial' : 'Add testimonial'}</h2></div><button type="button" className="icon" onClick={() => setEditingTestimonial(null)}>×</button></div><label>Quote<textarea required rows={6} value={editingTestimonial.quote || ''} onChange={e => setEditingTestimonial({ ...editingTestimonial, quote: e.target.value })} /></label><label>Name<input required value={editingTestimonial.name || ''} onChange={e => setEditingTestimonial({ ...editingTestimonial, name: e.target.value })} /></label><label>Activity<input required value={editingTestimonial.activity || ''} onChange={e => setEditingTestimonial({ ...editingTestimonial, activity: e.target.value })} /></label><div className="photo-editor"><div className="photo-preview">{editingTestimonial.photo_url ? <img src={editingTestimonial.photo_url} alt="" style={{objectPosition:`${editingTestimonial.photo_position_x ?? 50}% ${editingTestimonial.photo_position_y ?? 30}%`}} /> : <span>{editingTestimonial.name ? editingTestimonial.name.split(/\s+/).map(x => x[0]).join('').slice(0,2).toUpperCase() : 'PHOTO'}</span>}</div><div className="photo-controls"><label>Photo<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={e => setPhotoFile(e.target.files?.[0] || null)} /><small>JPG, PNG, WEBP or GIF · max 5MB</small></label><label>Horizontal position <span className="optional">{editingTestimonial.photo_position_x ?? 50}%</span><input type="range" min="0" max="100" value={editingTestimonial.photo_position_x ?? 50} onChange={e => setEditingTestimonial({ ...editingTestimonial, photo_position_x: Number(e.target.value) })} /></label><label>Vertical position <span className="optional">{editingTestimonial.photo_position_y ?? 30}%</span><input type="range" min="0" max="100" value={editingTestimonial.photo_position_y ?? 30} onChange={e => setEditingTestimonial({ ...editingTestimonial, photo_position_y: Number(e.target.value) })} /></label></div></div><div className="field-grid compact"><label>Status<select value={editingTestimonial.status || 'draft'} onChange={e => setEditingTestimonial({ ...editingTestimonial, status: e.target.value as 'draft' | 'published' })}><option value="draft">Draft</option><option value="published">Published</option></select></label><label>Display order<input type="number" min="0" value={editingTestimonial.sort_order ?? 0} onChange={e => setEditingTestimonial({ ...editingTestimonial, sort_order: Number(e.target.value) })} /></label></div><div className="modal-actions"><button type="button" className="secondary" onClick={() => setEditingTestimonial(null)}>Cancel</button><button className="primary" disabled={saving}>{saving ? 'Saving…' : 'Save testimonial'}</button></div></form></div>}
